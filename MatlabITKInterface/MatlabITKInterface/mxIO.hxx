...
mxIO::mxIO(const mxArray * mxInput_, mxArray ** mxOutput_):mxInput(mxInput_), mxOutput(mxOutput_){
    ...
    for(int i=0; i<mxGetNumberOfFields(mxInput); ++i)
        unusedFields.insert(std::string(mxGetFieldNameByNumber(mxInput, i)));
}
...
mxIO::~mxIO(){
    ...
    *mxOutput = mxCreateStructArray(2,dims,int(output.size()),const_cast<const char **>(&stringStarts[0]));
    ...
    if(!unusedFields.empty()){
        ...
        oss << "The following field(s) were not used : ";
        for(auto it=unusedFields.begin(); it!=unusedFields.end(); ++it){
            if(it!=unusedFields.begin()) oss << ", ";
            oss << *it;}
        oss << ".\n";
        MexMsg() << oss.str();
    }
...
mxIO::GetVector(std::string fieldName) const {
    ...
    typename FieldImageType::Pointer image = GetImageWOTranspose<FieldImageType>(fieldName);
//    MexMsg() << "GetVector fieldname " << fieldName << ", " << image << "\n";
...
//    return TransposeFirstTwoImageCoordinates<ImageType>(image);
    ...
    for(inputIt.GoToBegin(); !inputIt.IsAtEnd(); ++inputIt){
        ...
        IndexType outputIndex = inputIt.GetIndex();
        std::swap(outputIndex[0],outputIndex[1]);
...
std::vector<size_t>
mxIO::GetNumericArrayDimension(std::string fieldName) const {
    
    std::string errMsg = "GetNumericArrayDimension(" + fieldName + ") error : ";
    ...
    return std::vector<size_t>(mxSize, mxSize+ArrayDimension);
}
...
mxIO::GetImageWOTranspose(std::string fieldName) const
{
    ...
    std::vector<size_t> mxSize = GetNumericArrayDimension(fieldName);
    ...
    const mwSize ArrayDimension = mxSize.size();
    ...
    if(ImageFullDimension > ArrayDimension)
        itkExceptionMacro(<< errMsg << "Dimensions of image and array do not match. "
                          ...
                          << ArrayDimension);
    if(ImageFullDimension < ArrayDimension){
        for(long i=ImageFullDimension; i<ArrayDimension; ++i)
            if(mxSize[i]!=1)
                itkExceptionMacro(<< errMsg << "Dimensions of image and array do not match. "
                                  << "Respectively : " << FirstDimension + Dimension << " and "
...
//    MexMsg() << "Getting " << fieldName << ", Size : " << Size << ", buffer[0] " << Image->GetBufferPointer()[0] << " Image " << Image << "\n";
