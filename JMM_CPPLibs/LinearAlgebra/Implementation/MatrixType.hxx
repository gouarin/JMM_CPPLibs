...
template<typename TC, size_t VR, size_t VC> auto Matrix<TC,VR,VC>::
FromRows(const std::array<std::array<ComponentType, Columns>,Rows> & t) -> Matrix
{
    Matrix m;
...
template<typename TC, size_t VR, size_t VC> auto Matrix<TC,VR,VC>::
FromColumns(const std::array<std::array<ComponentType, Rows>,Columns> & t) -> Matrix
{
    Matrix m;
    for(int i=0; i<Rows; ++i)
        for(int j=0; j<Columns; ++j)
...
Matrix<TC,VR,VC>::Inverse() const
{
    ...
    switch (VR) {
        ...
        case 3:
            ...
            break;
        default: { // Gauss pivot, pasted from solve fct below
            const size_t n=VR;
            Matrix m=*this;
            b=Identity();
